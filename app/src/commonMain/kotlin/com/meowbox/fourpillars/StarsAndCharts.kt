...
data class ChartParametersImpl(
...
@Serializer(forClass = Star::class)
class Poop : KSerializer<Star> {
    override val descriptor: SerialDescriptor
        get() = StringDescriptor.withName("Dorth")

    override fun deserialize(input: Decoder): Star {
        try {
            return Star.valueOf(input.decodeString())
        } catch (exception: Exception) {
            print("I couldn't understand star: ${input.decodeString()}")
            throw exception
        }
    }

    override fun serialize(output: Encoder, obj: Star) {
        output.encodeString(obj.name)
    }
}

@Serializable(with = Poop2::class)
enum class Auspices {
    Exalted, Positive, Neutral, Mixed, Negative, Haunted
}


@Serializer(forClass = Auspices::class)
class Poop2 : KSerializer<Auspices> {
    override val descriptor: SerialDescriptor
        get() = StringDescriptor.withName("Dorth")

    override fun deserialize(input: Decoder): Auspices {
        try {
            return Auspices.valueOf(input.decodeString())
        } catch (exception: Exception) {
            print("I couldn't understand AUSPICES: ${input.decodeString()}")
            throw exception
        }
    }

    override fun serialize(output: Encoder, obj: Auspices) {
        output.encodeString(obj.name)
    }
}
...
expect class Ephemeris {
    fun lunarDate(year: Int, monthOfYear: Int, dayOfMonth: Int): LunarDate
    fun chart(lunarDate: LunarDate, hourOfDay: Int, minuteOfHour: Int): Chart
}
